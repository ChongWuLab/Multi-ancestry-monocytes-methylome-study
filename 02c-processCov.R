...
library(dplyr)
setwd("/rsrch5/home/biostatistics/wzhang24/data/Processed_data/cov/CAU/")
cov = as.data.frame(fread("/rsrch5/home/biostatistics/wzhang24/data/WGBS/CAU_310meta_w20Kwgbs.csv"))
...
cov = cov[,colnames(cov) %in% c("Age","BMI","Smoke","Alcohol_Drinking")]

ids = read.table("/rsrch5/home/biostatistics/wzhang24/data/Processed_data/ids/CAU/intersect_ids.txt") 
ids = ids$V1
cov = cov[ids,]

# add 5 genetic PCs
pc = as.data.frame(fread("/rsrch5/home/biostatistics/wzhang24/data/Processed_data/genetic_data/CAU/dataAll/pca.eigenvec"))
pc = pc[,2:12]
colnames(pc) = c("Subj","PC1","PC2","PC3","PC4","PC5","PC6","PC7","PC8","PC9","PC10")
#colnames(pc) = c("Subj","PC1","PC2","PC3","PC4","PC5")
rownames(pc) = pc$Subj
pc = pc[,-1]

#add ten non-genetic PCs
ng_pcs = readRDS("/rsrch5/home/biostatistics/wzhang24/data/Processed_data/nongenetic_PCs/CAU/pcs_most_variated.RDS") %>% as.data.frame()
ng_pcs = ng_pcs[,1:10]
...
qcovar = cov[,c("Age","BMI","PC1","PC2","PC3","PC4","PC5","ngPC1","ngPC2","ngPC3","ngPC4","ngPC5","ngPC6","ngPC7","ngPC8","ngPC9","ngPC10")]
